        ...
        match self {
            Operation::Function(mid, fid, types) => {
                ...
                let has_acquire_resources =
                    if let resources = func.get_acquires_global_resources() {
                        !resources.is_empty()
                    } else {
                        false
                    };
            ...
            Operation::PackVariant(module_id, datatype_id, variant_id, items) => todo!(),
            Operation::UnpackVariant(module_id, datatype_id, variant_id, items, ref_type) => todo!(),
        }
    }
}

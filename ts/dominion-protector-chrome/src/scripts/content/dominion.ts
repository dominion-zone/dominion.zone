...
function installProtector(origin: Wallet): Wallet | null {
  ...
  const checkTransaction = ({
    ...
    return new Promise((resolve, reject) => {
      window.addEventListener('message', event => {
        console.log('Dominion event', event);
        if (
          event.source !== window ||
          !event.data ||
        ...
        if (!event.data.approve) {
          return reject('Dominion protection has canceled the transaction');
        }
        resolve(event.data.approve);
      });
      window.postMessage({
        checkTransaction: {
          transaction: tx,
  ...
  if (origin.features['sui:signTransaction']) {
    ...
    const signTransaction = async ({
      ...
      await checkTransaction({tx, account, chain, signal});
...
async function trackTabWallets() {
  ...
  const register = (...wallets: Wallet[]) => {
    ...
    wallets = wallets.filter(
      wallet =>
        !registeredWalletsSet.has(wallet) &&
        !wallet?.id?.startsWith('DP:'),
    );
